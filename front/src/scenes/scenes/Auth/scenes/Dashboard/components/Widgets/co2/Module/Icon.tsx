...
class Icon extends Component<Props> {
    render() {
        const { size, service, index } = this.props;

        const view = service.getView;
        const data = view.getData(index);

        if (!data) return null;

        const iconClass = cn(styles.icon, {
            [styles.iconSmall]: size === ScreenSize.small,
            [styles.normal]: data.co2 <= 700,
            [styles.warning]: data.co2 > 700 && data.co2 <= 1000,
            [styles.alarm]: data.co2 > 1000,
        });
